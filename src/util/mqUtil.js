  ...
  var ClientImpl = function (uri, host, port, path, clientId) {
    ...
    for (var key in localStorage) {
      if (key.indexOf('Sent:' + this._localKey) == 0 ||
        key.indexOf('Received:' + this._localKey) == 0) { this.restore(key) }
    }
  }
  ...
  var Client = function (host, port, path, clientId) {
    ...
    this.subscribe = function (filter, subscribeOptions) {
      ...
      if (typeof subscribeOptions.qos !== 'undefined' &&
        !(subscribeOptions.qos === 0 || subscribeOptions.qos === 1 || subscribeOptions.qos === 2)) { throw new Error(format(ERROR.INVALID_ARGUMENT, [subscribeOptions.qos, 'subscribeOptions.qos'])) }
